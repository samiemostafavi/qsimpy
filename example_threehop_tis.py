...
if __name__ == "__main__":
    ...
    df["a_time_in_service1"] = df.apply(
        lambda row: (row.start_time - row.a_last_service_time1)
        if row.a_queue_is_busy1
        else None,
        axis=1,
    )
    df["a_time_in_service2"] = df.apply(
        lambda row: (row.start_time - row.a_last_service_time2)
        if row.a_queue_is_busy2
        else None,
        axis=1,
    )
    df["a_time_in_service3"] = df.apply(
        lambda row: (row.start_time - row.a_last_service_time3)
        if row.a_queue_is_busy3
        else None,
        axis=1,
    )
    ...
    df["b_time_in_service2"] = df.apply(
        lambda row: (row.queue2_queue_time - row.b_last_service_time2)
        if row.b_queue_is_busy2
        else None,
        axis=1,
    )
    df["b_time_in_service3"] = df.apply(
        lambda row: (row.queue2_queue_time - row.b_last_service_time3)
        if row.b_queue_is_busy3
        else None,
        axis=1,
    )
    ...
    df["c_time_in_service3"] = df.apply(
        lambda row: (row.queue3_queue_time - row.c_last_service_time3)
        if row.c_queue_is_busy3
        else None,
        axis=1,
    )
    ...
    n2 = 2
    values_count = (
        df[["a_queue_length1", "a_queue_length2", "a_queue_length3"]]
        .value_counts()[:n]
        .index.tolist()
    )
